...
- (void)scanWithCompletionHandler:(HLYScanPeripheralsCompletionHandler)completionHandler {
    [self.bluetoothManager scanPeripheralsWithCompletionHandler:completionHandler];
}

- (void)connectPeripheral:(CBPeripheral *)peripheral completionHandler:(void(^)(NSError *error))completionHandler {
    
    [self.characteristics removeAllObjects];
    
    __weak typeof(self) wSelf = self;
    [self.bluetoothManager connectPeripheral:peripheral completionHandler:^(CBService *service, NSError *error) {
...
- (void)sendData:(NSData *)data completionHandler:(void(^)(NSError *error))completionHandler {
    ...
    if (self.isConnected) {
        ...
        // 自动连接
//        self.bluetoothManager connectPeripheral:(CBPeripheral *) completionHandler:<#^(CBService *service, NSError *error)completionHandler#>
        error = [NSError errorWithDomain:@"HLYBluetoothPrint" code:0 userInfo:@{NSLocalizedDescriptionKey : @"Printer not connected"}];
    }
    if (completionHandler) {
        completionHandler(error);
    }
